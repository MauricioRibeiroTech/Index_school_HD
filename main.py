...
if st.session_state["authentication_status"]:
    ...
    SIM = ['Sim 1', 'Sim 2', 'Sim 3', 'Sim 4', 'Sim 5', 'Sim 6', 'Sim 7', 'Sim 8', 'Sim 9', 'Sim 10', 'Sim 11',
           'Sim 12']
    ...
    if uploaded_file is not None:
        ...
        with st.sidebar:
            ...
            componente_selecionada = st.radio("Componente", ["Matematica"])
        ...
        # st.table(df)
        st.bar_chart(df, x='Nomes', y=SIM, stack=False)

        a, b, c = st.columns(3)

        media_1 = df['Sim 1'].mean().round(1)
        media_2 = df['Sim 2'].mean().round(1)
        media_3 = df['Sim 3'].mean().round(1)
        ...
        a.metric("Simulado 1", media_1, dif1.round(1), border=True)
        b.metric("Simulado 2", media_2, dif2.round(1), border=True)
        c.metric("Simulado 3", media_3, dif3.round(1), border=True)
